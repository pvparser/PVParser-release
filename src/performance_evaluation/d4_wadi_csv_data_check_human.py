...
class CSVDataHumanChecker:
    ...
    def __init__(self, time_window: float, time_shift: float, csv_timezone: str, current_timezone: str, match_threshold: float):
        """
    ...
    def find_closest_match_in_time_window_before(self, target_time: float, parsed_df: pd.DataFrame, column_name: str, parsed_timestamp_col: str,
        ...
        time_diff = parsed_df[parsed_timestamp_col] - target_time + self.time_shift  # the time shift is added to the target time
        ...
        ts_series = window_data[parsed_timestamp_col]
        signed_time_diff = ts_series - target_time + self.time_shift  # the time shift is added to the target time
    ...
    def find_best_direction_match(self, aligned_original_df: pd.DataFrame, aligned_parsed_df: pd.DataFrame, 
        ...
        before_score = before_results['time_window_match_rate'] if before_results['total_values'] > 0 else 0
        after_score = after_results['time_window_match_rate'] if after_results['total_values'] > 0 else 0
    ...
    def _test_direction_matches(self, aligned_original_df: pd.DataFrame, aligned_parsed_df: pd.DataFrame,
        ...
        for idx, row in aligned_original_df.iterrows():
            ...
            # if idx == 11198:
            #     print(f"target_time: {target_time}, original_value: {original_value}")
    ...
    def align_time_ranges(self, original_df: pd.DataFrame, parsed_df: pd.DataFrame, 
        ...
        original_start = parsed_min + self.time_shift + self.time_window
        original_end = parsed_max + self.time_shift - self.time_window
    ...
    def _values_equal(self, val1: Any, val2: Any, tolerance: float = 0.05) -> bool:
        """
        ...
        Args:
            val1: First value
            val2: Second value
            tolerance: Default tolerance (overridden by dynamic tolerance based on values)
        ...
        try:
            if isinstance(val1, (int, float, np.integer, np.floating)) and isinstance(val2, (int, float, np.integer, np.floating)):
                # Dynamic tolerance based on minimum value
                min_val = min(abs(float(val1)), abs(float(val2)))
                if min_val > 100:
                    tolerance = 0.01
                elif min_val > 1:
                    tolerance = 0.005
    ...
    def load_original_data(self, original_csv_path: str, timestamp_col: str) -> pd.DataFrame:
        ...
        _, _, original_df, _ = load_inference_constraints(original_csv_path, use_data_constraint_manager=False, use_heuristic_constraint_manager=False, dataset_name="wadi_enip")
        ...
        if all_file_results:
            ...
            for file in all_file_results:
                for col_name, col_stats in file['column_results'].items():
                    ...
                    unmatched_count = max(total_values - time_window_matches, 0)
...
def main():
    ...
    time_window = 60  # 10 seconds, the time window for the human check
    time_shift = 420  # 10 minutes, the time shift between the original and parsed data
    
    dataset_name = "wadi_enip"
    combination_payloads_folder = "wadi_capture_00043_00052"
    parsed_csv_folder = "wadi_capture_00087_00091"
    scada_ip = "192.168.1.67"
    original_csv_path = f"dataset/{dataset_name}/physics/{dataset_name}_evaluation.csv"
    csv_timezone = 'UTC'
    ...
    checker = CSVDataHumanChecker(time_window=time_window, time_shift=time_shift, csv_timezone=csv_timezone, current_timezone=current_timezone, match_threshold=0.90)
    ...
    print(f"Time window: {time_window} seconds, Time shift: {time_shift} seconds")
