...
class PayloadInferenceSelector:
    ...
    def match_groups_with_sessions_greedy(self, output_file: str = None, max_workers: int = None) -> Dict[str, Any]:
        """
        ...
        parallel_args = []
        for group_key, dynamic_columns in self.dynamic_columns_by_groups.items():
            # if group_key != "HMI_2":
            #     continue
            ...
            for session_file, session_key in parsed_sessions:
                ...
                #     continue
                args = (
                    group_key, dynamic_columns, session_file, session_key,
        ...
        if output_file:
            try:
                self.results_io.save_matching_results(selected_results, output_file)
            except Exception as e:
                print(f"Warning: Failed to save matching results via MergeResultsIO: {e}")
    ...
    def _apply_greedy_selection(self, candidates: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for cand in candidates:
            g = cand['group_key']
            s = cand['session_key']
            if g in used_groups or s in used_sessions:
                continue
            selected_results[g] = {
    ...
    def _find_best_solution_match_for_group(self, solutions: List[MergedSolution], session_key: str, group_key: str, dynamic_columns: List[str]) -> Dict[str, Any]:
        """
        ...
        for solution_index, solution in enumerate(solutions):
            ...
            matched_mapping, matched_scores, unmatched_fields, unmatched_columns = self._calculate_field_column_matching(session_key, inferred_fields, dynamic_columns)
        ...
        if best_solution_match:
            ...
            unmatched_column_scores = self._calculate_unmatched_column_scores(session_key, inferred_fields, best_solution_match['matched_mapping'], dynamic_columns)
            best_solution_match['unmatched_column_scores'] = unmatched_column_scores
        
        return best_solution_match

    def _calculate_unmatched_column_scores(self, session_key: str, inferred_fields: List[Dict[str, Any]], matched_mapping: Dict[str, str], dynamic_columns: List[str]) -> Dict[str, Any]:
        """
        ...
        fields_to_match = self._preprocess_inferred_fields(session_key, inferred_fields)
    ...
    def _preprocess_inferred_fields(self, session_key: str, inferred_fields: List[Dict[str, Any]]) -> Dict[str, Tuple[str, List[float]]]:
        """
        ...
        Args:
            session_key: Session identifier to load payloads for
        ...
        try:
            # Convert session_key string back to tuple and extract the first element
            # session_key is in format "(('192.168.1.10', '192.168.1.200', 6), 34, 123)"
            # We want to extract ('192.168.1.10', '192.168.1.200', 6)
            try:
            ...
            # Load payloads for the given session key (use CSV average sampling frequency when available)
            json_file = f"src/data/protocol_field_inference/{self.dataset_name}/data_payloads_combination/{self.combination_payloads_folder}/{actual_session_key}_top_0_data_payloads_combination.json"
            ...
            if not whole_payloads:
                print(f"Warning: No payload data available for session {actual_session_key}")
                return {}
    ...
    def _calculate_field_column_matching(self, session_key: str, inferred_fields: List[Dict[str, Any]], dynamic_columns: List[str]) -> Tuple[float, Dict[str, str], List[Dict[str, Any]], List[str]]:
        """
        ...
        # Preprocess inferred fields to create fields_to_match (loads payloads by session key)
        fields_to_match = self._preprocess_inferred_fields(session_key, inferred_fields)
...
def _compute_group_session_score(args):
    ...
    try:
        ...
        best_solution_match = selector._find_best_solution_match_for_group(
            session_data['solutions'], session_key, group_key, dynamic_columns
        )
...
def generate_alignment_columns_fields(dataset_name: str, alignment_result_path: str, non_overlap_blocks_folder: str, output_path: str = None):
    ...
    for group_id, alignment_result in alignment_results.items():
        ...
        # Parse session key to get actual session tuple
        actual_session_key = session_key
        if isinstance(session_key, str) and session_key.startswith('((') and ',' in session_key:
            # Parse the string to extract the tuple part
            try:
                import ast
                parsed = ast.literal_eval(session_key)
                if isinstance(parsed, tuple) and len(parsed) >= 1:
                    actual_session_key = parsed[0]
            except:
                print(f"Warning: Could not parse session_key {session_key}, using as-is")
...
def main():
    ...
    combination_payloads_folder = "Dec2019_00000_00003_300_02"
    ...
    continuous_threshold = 0.55  # Higher threshold for continuous data
    enumeration_threshold = 0.55  # Lower threshold for enumeration data
    base_correlation_threshold = 0.55  # Base threshold for internal evaluation
    ...
    alignment_result_file = f"src/data/payload_inference/{dataset_name}/payload_inference_alignment/alignment_results/{combination_payloads_folder}_{suffix}/{dataset_name}_{scada_ip}_alignment_result.json"
    max_workers = 1
    matching_results = selector.match_groups_with_sessions_greedy(output_file=alignment_result_file, max_workers=max_workers)
    
    print(f"\nMatching completed. Found {len(matching_results)} matches.")
