...
class ScenarioConfig:
    ...
    def traffic_root(self) -> Path:
        return self.hist_root.parent / "supervisory_traffic_attack"


SCENARIOS: dict[str, ScenarioConfig] = {
    "s2": ScenarioConfig(
        scenario="S2",
        hist_root=Path("/dataTEN/attack_detection_data/s2/supervisory_historian_attack"),
        control_root=Path("/dataTEN/attack_detection_data/s2/control_attack"),
    ...
    "s3": ScenarioConfig(
        scenario="S3",
        hist_root=Path("/dataTEN/attack_detection_data/s3/supervisory_historian_attack"),
        control_root=Path("/dataTEN/attack_detection_data/s3/control_attack"),
    ...
    "s4": ScenarioConfig(
        scenario="S4",
        hist_root=Path("/dataTEN/attack_detection_data/s4/supervisory_historian_attack"),
        control_root=Path("/dataTEN/attack_detection_data/s4/control_attack"),
        sup_channels=("P101.Status", "MV201.Status", "FIT301.Pv", "LIT301.Pv"),
        control_specs={
            "MV201.Status@ctrl": ControlTraceSpec(
                value_key="MV201.Status",
    ...
    "s5": ScenarioConfig(
        scenario="S5",
        hist_root=Path("/dataTEN/attack_detection_data/s5/supervisory_historian_attack"),
        control_root=Path("/dataTEN/attack_detection_data/s5/control_attack"),
        sup_channels=("P101.Status", "MV201.Status", "FIT201.Pv", "LIT301.Pv"),
        control_specs={
            "MV201.Status@ctrl": ControlTraceSpec(
                value_key="MV201.Status",
...
def historian_data_root(config: ScenarioConfig) -> Path:
    candidate = config.hist_root / "data"
    return candidate if candidate.exists() else config.hist_root


def control_data_root(config: ScenarioConfig) -> Path:
    candidate = config.control_root / "data"
    return candidate if candidate.exists() else config.control_root
...
def load_sup_split(config: ScenarioConfig, split_name: str, channels: tuple[str, ...]) -> pd.DataFrame:
    if split_name in {"training", "test_base"}:
        path = historian_data_root(config) / f"{split_name}.csv"
    else:
        path = historian_data_root(config) / f"{split_name}.csv"
...
def load_control_rec_split(
    ...
    split_root = control_data_root(config) / split_name
...
def load_attack_log(config: ScenarioConfig) -> pd.DataFrame:
    path = historian_data_root(config) / "attack_injection_log.csv"
...
def train_pasada_model(
    ...
    weights = np.sqrt(singular_values / np.sum(singular_values))
