#!/usr/bin/env python3
"""
Inject supervisory-historian attack4 CSVs into control-layer PCAPs using
frequent-pattern alignment results only.

This script is intentionally kept close to the attack2 control generator:
- it can also export the corresponding control-layer ``training.csv`` window as original traffic plus PV parsing
- each attack group points to one fp ``session_group``
- one fp group can contribute one or more PVs from the same exact session
- all resolved carrier sessions are injected into the corresponding
  ``period_identification/raw/<raw_folder>/<session>.pcap`` files

Current default groups:
- PLC20 ``S-110-0x006F_0xCC`` session for ``MV201.Status``
- PLC20 ``S-152-0x0070_0xCC`` session for ``FIT201.Pv``
- PLC30 session between ``('192.168.1.20', '192.168.1.30', 6)`` for
  ``LIT301.Pv``

``P101.Status`` is intentionally not included here because it does not yet have
an fp alignment result in the current repository state.
"""
...
from attack_detection.s3.supervisory_traffic_attack.s3_attack4_generation import (
    AttackPVSpec,
    FieldSpec,
    FlowMatchSpec,
    PacketFeatureSpec,
...
def _derive_flow_from_direction(base_session_key: str, direction: str) -> FlowMatchSpec:
...
def _derive_plc_label(flow: FlowMatchSpec) -> str:
    return f"PLC{flow.src_ip.split('.')[-1]}"


def _resolve_exact_payload_combination_path(
    dataset_name: str,
...
def build_attack_pv_spec_from_fp_alignment(
    ...
    alignment_result_data = _load_json(alignment_result_path)
    columns_fields = _load_json(alignment_columns_fields_path)
    pv_entry = columns_fields.get(pv_name)
    if not pv_entry:
        raise KeyError(f"PV {pv_name!r} not found in alignment columns fields: {alignment_columns_fields_path}")
    ...
    payload_combination_path = _resolve_exact_payload_combination_path(
    ...
    if not payload_combination_path.exists():
        raise FileNotFoundError(f"Payload combination file not found: {payload_combination_path}")
    ...
    flow = _derive_flow_from_direction(base_session_key, direction)
...
def build_attack_pv_specs_from_fp_groups(
    ...
    for index, group in enumerate(attack_groups):
        ...
        for pv_name in normalized_pv_names:
            if pv_name in per_pv_metadata:
                raise ValueError(f"Duplicate PV configured across attack_groups: {pv_name}")
            ...
            session_to_pv_names.setdefault(metadata["base_session_key"], []).append(pv_name)
...
def _default_historian_attack4_dir() -> Path:
    return SRC_ROOT / "attack_detection" / "s3" / "supervisory_historian_attack" / "data"


def _default_control_attack4_dir() -> Path:
    return SRC_ROOT / "attack_detection" / "s3" / "control_attack" / "data"
...
def _resolve_attack_csv_paths(
    ...
    if attack_csv_names is None:
        raise ValueError(
            "Please explicitly choose which historian attack4 CSVs to inject. "
            "Set attack_csv_path to one file, or attack_csv_names to a list such as "
            "['test_00.csv', 'test_01.csv']."
        )

    csv_dir = Path(attack_csv_dir) if attack_csv_dir is not None else _default_historian_attack4_dir()
    if not csv_dir.exists():
        raise FileNotFoundError(
            f"Historian attack4 CSV directory not found: {csv_dir}. "
            "Run the s3 supervisory historian generator first or set attack_csv_dir manually."
        )
    ...
    if missing:
        raise FileNotFoundError(
            "Some requested historian attack4 CSV files were not found:\n  "
            + "\n  ".join(missing)
        )
...
def _normalize_batch_output_names(batch_report: Dict[str, Any]) -> None:
    for file_report in batch_report.get("files", []):
        ...
        normalized_pcap_output_path = pcap_output_path.with_name(
            f"{canonical_stem}_injected{pcap_output_path.suffix}"
        )
        normalized_pcap_output_path = _rename_if_needed(
            pcap_output_path,
            normalized_pcap_output_path,
        )
        ...
        for plc_label, plc_csv_path in (file_report.get("plc_upload_csv_paths", {}) or {}).items():
            ...
            normalized_plc_csv_path = _rename_if_needed(
                plc_csv_path_obj,
                normalized_plc_csv_path,
            )
...
def _write_plc_csv_curve_plot(
    ...
    if output_path is None:
        output_path = str(
            Path(plc_csv_path).with_name(f"{Path(plc_csv_path).stem}_curves.png")
        )
    ...
    return [
        {
            ...
            "packet_feature": {
                ...
                "expected_bytes_hex": None
                if not attack_spec.packet_feature
                or attack_spec.packet_feature.expected_bytes is None
                else attack_spec.packet_feature.expected_bytes.hex(),
                "expected_mask_hex": None
                if not attack_spec.packet_feature
                or attack_spec.packet_feature.expected_mask is None
...
def inject_historian_attack4_into_control_pcaps(
    *,
    dataset_name: str,
    raw_folder_names: Sequence[str],
    attack_groups: Sequence[Dict[str, Any]],
    ...
    base_output_dir = (
        ...
        else _default_control_attack4_dir()
    ).resolve()
    ...
    if training_raw_folder_names:
        ...
        if len(resolved_training_csv_paths) != 1:
            raise ValueError(
                "Control attack4 training baseline expects exactly one historian CSV."
            )
    ...
    if test_base_raw_folder_names:
        ...
        if len(resolved_test_base_csv_paths) != 1:
            raise ValueError(
                "Control attack4 test_base baseline expects exactly one historian CSV."
            )
...
def main() -> None:
    print("Control Attack4 Injection")
    print("=" * 50)
    ...
    attack_groups = [
        ...
        {
            "session_group": "swat_('192.168.1.10', '192.168.1.20', 6)_20__S-152-0x0070_0xCC__g-2",
            "combination_payloads_folder": fp_combination_payloads_folder,
            "pv_names": ["FIT201.Pv"],
        },
        {
    ...
    # Default: generate clean training/test_base baselines plus the full attack4 test set.
    ...
    # attack_csv_path = "src/attack_detection/s3/supervisory_historian_attack/data/test_00.csv"
    ...
    show_progress = True
    # Attack4 control regeneration is expensive. Use a moderate worker count so
    # the default run remains practical without pushing memory as hard as the
    # full auto-detected process count.
    max_workers = 4

    inject_historian_attack4_into_control_pcaps(
        dataset_name=dataset_name,
        raw_folder_names=raw_folder_names,
        attack_groups=attack_groups,
        attack_csv_path=attack_csv_path,
