"""
Build outputs directly under ``../data/`` for the s3 supervisory-historian script.
...
try:
    ...
    from .s3_attack4_FIT201_template import build_fit201_attack_tail, load_fit201_template
    from .s3_attack4_LIT301_template import build_lit301_attack_tail, load_lit301_template
    from .s3_attack4_MV201_template import build_mv201_attack_tail, load_mv201_template
    from .s3_attack4_P101_template import build_p101_attack_tail, load_p101_template
except ImportError:
    from attack_injection_logging import write_attack_injection_log
    from s3_attack4_FIT201_template import build_fit201_attack_tail, load_fit201_template
    from s3_attack4_LIT301_template import build_lit301_attack_tail, load_lit301_template
...
def export_attack4_dataset(
    ...
    *,
        ...
        test_files_list.append(
            {
                ...
                "attack_point_id": i,
                "note": (
                    "One coordinated injection per file; all attacked columns fully "
                    "replace every row from the shared start to the test end."
                ),
            }
        )
    ...
    meta: Dict[str, Any] = {
        "description": (
            "Metadata for s3 supervisory_historian_attack under manipulated-PV "
...
def main() -> None:
    ...
    saved = plot_all_tests_line_chart(
        ...
        save_path=plot_path,
        title="Attack4: test_base (dashed) vs attacked test_XX (per column)",
    )
    print(f"Wrote {saved}")
