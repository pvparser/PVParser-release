...
from attack_detection.s5.supervisory_historian_attack.attack_injection_logging import (
    CSV_FILENAME,
    JSON_FILENAME,
    ...
    if _SCAPY_CACHE is None:
        try:
            ...
            raise ModuleNotFoundError(
                "scapy is required for s5 supervisory traffic generation. "
...
def _default_attack4_output_dir(attack_label: Optional[str] = None) -> str:
    """Return the default output directory for generated attack4 traffic artifacts."""
...
def _resolve_output_path(
    ...
    *,
    ...
    """
    Resolve an output path under the current ``attack4`` directory by default.
    ...
    """
    base_dir = Path(_default_attack4_output_dir(attack_label))
...
def _resolve_output_dir(
    ...
    *,
    ...
    """
    Resolve an output directory under the current ``attack4`` directory by default.
    """
    base_dir = Path(_default_attack4_output_dir(attack_label))
    ...
    *,
    ...
    if attack_csv_paths:
        ...
        base_dir = (
            ...
            else repo_root / Path("src/attack_detection/s5/supervisory_historian_attack/data")
        )
...
def _is_attacked_test_csv_path(csv_path: str | os.PathLike[str]) -> bool:
    stem = Path(csv_path).stem
    return stem.startswith("test_") and stem[5:].isdigit()


def _default_attack4_supervisory_view_csv_dir(attack_label: str) -> Path:
    return (
        Path(_default_attack4_output_dir())
        / "_supervisory_view_attack_csv"
        / attack_label
    )


def _materialize_supervisory_view_attack_csv(
    attack_csv_path: str | os.PathLike[str],
) -> Tuple[str, Dict[str, Any]]:
    source_attack_csv_path = Path(attack_csv_path).resolve()
    view_info: Dict[str, Any] = {
        "attack_csv_view_mode": "passthrough",
        "replaced_columns": [],
        "source_attack_csv_path": str(source_attack_csv_path),
        "effective_attack_csv_path": str(source_attack_csv_path),
    }
    if not _is_attacked_test_csv_path(source_attack_csv_path):
        return str(source_attack_csv_path), view_info

    test_base_csv_path = source_attack_csv_path.parent / "test_base.csv"
    if not test_base_csv_path.exists():
        raise FileNotFoundError(
            "Expected sibling test_base.csv for supervisory-view attack CSV construction: "
            f"{test_base_csv_path}"
        )

    attack_df = pd.read_csv(source_attack_csv_path)
    test_base_df = pd.read_csv(test_base_csv_path)
    if len(attack_df) != len(test_base_df):
        raise ValueError(
            "attack CSV and test_base CSV length mismatch for supervisory-view construction: "
            f"{source_attack_csv_path} ({len(attack_df)}) vs {test_base_csv_path} ({len(test_base_df)})"
        )

    replaced_columns = [
        column
        for column in MANIPULATED_PV_COLUMNS
        if column in attack_df.columns and column in test_base_df.columns
    ]
    if not replaced_columns:
        return str(source_attack_csv_path), view_info

    effective_dir = _default_attack4_supervisory_view_csv_dir(source_attack_csv_path.stem)
    effective_dir.mkdir(parents=True, exist_ok=True)
    effective_attack_csv_path = effective_dir / source_attack_csv_path.name
    effective_df = attack_df.copy()
    for column in replaced_columns:
        effective_df[column] = test_base_df[column].to_numpy(copy=False)
    effective_df.to_csv(effective_attack_csv_path, index=False)

    for filename in (CSV_FILENAME, JSON_FILENAME):
        source_label_path = source_attack_csv_path.parent / filename
        if source_label_path.exists():
            shutil.copy2(source_label_path, effective_dir / filename)

    view_info = {
        "attack_csv_view_mode": SUPERVISORY_ATTACK_VIEW_MODE,
        "replaced_columns": replaced_columns,
        "source_attack_csv_path": str(source_attack_csv_path),
        "effective_attack_csv_path": str(effective_attack_csv_path.resolve()),
        "test_base_csv_path": str(test_base_csv_path.resolve()),
    }
    return str(effective_attack_csv_path.resolve()), view_info


def _annotate_supervisory_view_attack_report(
    report: Dict[str, Any],
    *,
    view_info: Dict[str, Any],
) -> Dict[str, Any]:
    source_attack_csv_path = view_info["source_attack_csv_path"]
    effective_attack_csv_path = view_info["effective_attack_csv_path"]
    report["source_attack_csv_path"] = source_attack_csv_path
    report["effective_attack_csv_path"] = effective_attack_csv_path
    report["attack_csv_view_mode"] = view_info["attack_csv_view_mode"]
    report["supervisory_clean_replaced_columns"] = list(view_info["replaced_columns"])
    if report.get("attack_csv_path") == effective_attack_csv_path:
        report["attack_csv_path"] = source_attack_csv_path

    attack_log = report.get("attack_injection_log")
    if isinstance(attack_log, dict):
        attack_log["source_attack_csv_path"] = source_attack_csv_path
        attack_log["effective_attack_csv_path"] = effective_attack_csv_path
        attack_log["attack_csv_view_mode"] = view_info["attack_csv_view_mode"]
        attack_log["supervisory_clean_replaced_columns"] = list(
            view_info["replaced_columns"]
        )

    for file_report in report.get("files", []) or []:
        if file_report.get("attack_csv_path") == effective_attack_csv_path:
            file_report["attack_csv_path"] = source_attack_csv_path
        file_report["source_attack_csv_path"] = source_attack_csv_path
        file_report["effective_attack_csv_path"] = effective_attack_csv_path
        file_report["attack_csv_view_mode"] = view_info["attack_csv_view_mode"]
        file_report["supervisory_clean_replaced_columns"] = list(
            view_info["replaced_columns"]
        )
    return report
...
def build_supervisory_attack4_pv_specs() -> List[AttackPVSpec]:
    """
    Return the default supervisory packet specs for attack4.

    These defaults are derived from the repository's supervisory alignment outputs
    under ``src/data/payload_inference/swat/payload_inference_alignment`` and use
    the same session-frame lengths already validated by the attack2 traffic
    generator. For PLC30 the carrier packets are in the PLC-to-historian
    direction even though one alignment artifact is keyed by the reverse session.
    """
...
def inject_attack_csv_into_pcap(
    ...
    *,
    ...
    pcap_output_path:
        ...
        ``s5/supervisory_traffic_attack/data/``.
    flow:
...
def inject_attack_csv_into_pcap_multi(
    ...
    *,
    ...
    _write_pv_curve_plot(
        pv_reports,
        resolved_curve_plot_path,
        title=f"Traffic attack4: original vs modified ({Path(pcap_input_path).name})",
    )
...
def build_attack4_traffic(
    ...
    *,
    ...
    """
    effective_attack_csv_path, view_info = _materialize_supervisory_view_attack_csv(
        attack_csv_path
    )
    is_multi_input = not isinstance(pcap_input_path, (str, os.PathLike))
    if pv_specs is not None:
        if is_multi_input:
            report = inject_attack_csv_into_pcap_inputs_multi(
                pcap_input_path,
                effective_attack_csv_path,
            ...
            return _annotate_supervisory_view_attack_report(report, view_info=view_info)

        input_path = Path(pcap_input_path)
        if input_path.is_dir():
            report = inject_attack_csv_into_pcap_folder_multi(
                pcap_input_path,
                effective_attack_csv_path,
                pcap_output_dir=pcap_output_path,
                pv_specs=pv_specs,
            ...
            return _annotate_supervisory_view_attack_report(report, view_info=view_info)
        report = inject_attack_csv_into_pcap_multi(
            pcap_input_path,
            effective_attack_csv_path,
            pcap_output_path,
            pv_specs=pv_specs,
    ...
    if is_multi_input:
        report = inject_attack_csv_into_pcap_inputs(
            pcap_input_path,
            effective_attack_csv_path,
            pcap_output_dir=pcap_output_path,
            protocol_type=protocol_type,
    ...
    if input_path.is_dir():
        report = inject_attack_csv_into_pcap_folder(
            pcap_input_path,
            effective_attack_csv_path,
            pcap_output_dir=pcap_output_path,
            protocol_type=protocol_type,
    ...
    report = inject_attack_csv_into_pcap(
        pcap_input_path,
        effective_attack_csv_path,
        pcap_output_path,
        protocol_type=protocol_type,
...
def main() -> None:
    ...
    pv_specs = build_supervisory_attack4_pv_specs()
    ...
    for group in traffic_generation_groups:
        ...
        resolved_attack_csv_paths = _resolve_attack_csv_paths(
            ...
            attack_csv_dir=group.get("attack_csv_dir", "src/attack_detection/s5/supervisory_historian_attack/data"),
            ...
            run_summaries.append(
                {
                    ...
                    "source_attack_csv_path": report.get("source_attack_csv_path"),
                    "effective_attack_csv_path": report.get("effective_attack_csv_path"),
                    "attack_csv_view_mode": report.get("attack_csv_view_mode"),
                    "supervisory_clean_replaced_columns": report.get("supervisory_clean_replaced_columns"),
    ...
    summary_path = repo_root / Path(
        "src/attack_detection/s5/supervisory_traffic_attack/data/traffic_attack_summary.json"
    )
    summary = {
        "dataset_name": dataset_name,
