...
- inject one or more selected PVs from ``supervisory_historian_attack/data/test_XX.csv``
...
from attack_detection.s2.supervisory_traffic_attack.s2_attack2_generation import (
    AttackPVSpec,
    FieldSpec,
    FlowMatchSpec,
    PacketFeatureSpec,
...
def _default_historian_attack2_dir() -> Path:
    return SRC_ROOT / "attack_detection" / "s2" / "supervisory_historian_attack" / "data"


def _default_control_attack2_dir() -> Path:
    return SRC_ROOT / "attack_detection" / "s2" / "control_attack" / "data"
...
def _resolve_attack_csv_paths(
    ...
    if not csv_dir.exists():
        raise FileNotFoundError(
            ...
            "Run the s2 supervisory historian generator first or set attack_csv_dir manually."
        )
...
def main() -> None:
    ...
    # attack_csv_path = "src/attack_detection/s2/supervisory_historian_attack/data/test_00.csv"
    # attack_csv_names = None

    # Leave as None to save under src/attack_detection/s2/control_attack/data.
