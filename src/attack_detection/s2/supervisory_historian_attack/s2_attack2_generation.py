...
try:
    ...
    from .s2_attack2_FIT301_template import (
        FIT301_LEARNED_RISE_STAGE,
        build_fit301_attack_tail,
        load_fit301_template,
    )
    from .s2_attack2_LIT301_template import (
    ...
    from .s2_attack2_MV201_template import build_mv201_attack_tail, load_mv201_template
    from .s2_attack2_P101_template import build_p101_attack_tail, load_p101_template
except ImportError:
    from attack_injection_logging import write_attack_injection_log
    from s2_attack2_FIT301_template import (
        FIT301_LEARNED_RISE_STAGE,
    ...
    *,
    ...
    for i, s_rel in enumerate(starts_rel.tolist()):
        ...
        for col in columns_ordered:
            _assign_column_tail(tdf, col, s_rel, sequences[col])
            row_meta = {
        ...
        tdf.to_csv(os.path.join(output_dir, fname), index=False)
        ...
        test_files_list.append(
            {
                ...
                "attack_point_id": i,
                "note": (
                    "One coordinated injection per file; LIT301 is directly injected, "
                    "and the other attack-relevant PVs replay the same prototype's "
                    "natural response from that shared attack start."
                ),
            }
        )
    ...
    meta: Dict[str, Any] = {
        "description": (
            "Metadata for s2 supervisory_historian_attack under manipulated-PV "
        ...
        "attack_waveform_by_column": {
            "LIT301.Pv": {
                ...
                "template_module": "s2_attack2_LIT301_template.py",
                ...
                "entry_transition": (
                    "strict_template_attack_segment_then_shifted_post_attack_recovery"
                ),
            ...
            "MV201.Status": {
                ...
                "type": "template_plus_continuation_to_test_end",
                "template_module": "s2_attack2_MV201_template.py",
                "template_csv": os.path.abspath(template_csv_path),
            ...
            "P101.Status": {
                "role": "directly_affected_pv",
                "type": "template_plus_continuation_to_test_end",
                "template_module": "s2_attack2_P101_template.py",
                "template_csv": os.path.abspath(template_csv_path),
            ...
            "FIT301.Pv": {
                "role": "directly_affected_pv",
                "type": "template_plus_continuation_to_test_end",
                "template_module": "s2_attack2_FIT301_template.py",
                "template_csv": os.path.abspath(template_csv_path),
...
def main() -> None:
    ...
    saved = plot_all_tests_line_chart(
        ...
        save_path=plot_path,
        title="Attack2: test_base (dashed) vs attacked test_XX (per column)",
    )
    print(f"Wrote {saved}")
